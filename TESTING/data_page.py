...
class DataPage(Page) :
    def __init__(self, *args, **kwargs):
        ...
        self.menu_frame = ctk.CTkFrame(self, corner_radius=5)
        self.menu_frame.grid(row=0, column=0, rowspan=4, columnspan=2, padx=(100,5), pady=20, sticky="nsew")
        self.menu_frame.grid_columnconfigure(0, weight=1)
        
        self.delete_text = ctk.CTkLabel(master=self.menu_frame, text="Input the years to grab:\nie. 1980-1984,1987,2014-2012,2022")
        self.delete_text.grid(row=0, column=0, columnspan=2, pady=(20,10), sticky="ew")
        
        self.retrieve_text = ctk.CTkTextbox(self.menu_frame, height=100)
        self.retrieve_text.insert("0.0", "1980-1984,1987,2014-2012,2022")
        self.retrieve_text.grid(row=1, column=0, columnspan=2, padx=30, pady=(5,10), sticky="nsew")
        ...
        self.retrieve_data.grid(row=2, column=0, columnspan=2, padx=10, pady=10)
        
        self.current_text = ctk.CTkLabel(master=self.menu_frame, text="Input : N/A")
        self.current_text.grid(row=3, column=0, padx=10, pady=(10,0), sticky="ew")
        
        self.progress_text = ctk.CTkLabel(master=self.menu_frame, text="Progress : 0/0")
        self.progress_text.grid(row=4, column=0, padx=10, pady=5, sticky="ew")
        
        self.delete_text = ctk.CTkLabel(master=self.menu_frame, text="Delete the selected years")
        self.delete_text.grid(row=5, column=0, columnspan=2, pady=(100,0), sticky="ew")
        
        self.delete_data = ctk.CTkButton(self.menu_frame, text="Delete Data", command=self.delete_selected)
        self.delete_data.grid(row=6, column=0, columnspan=2, padx=20, pady=(5,5))
    ...
    def delete_selected(self):
        #self.scrollable_frame = ctk.CTkScrollableFrame(self, label_text="Data Years", width=180)
        #self.scrollable_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
    ...
    def update_movie_list(self):
        ...
        self.scrollable_frame.grid(row=0, column=2, rowspan=4, columnspan=2, padx=(0,100), pady=20, sticky="nsew")
        self.scrollable_frame.grid_columnconfigure(0, weight=1)
